...
import com.example.barry215.woo.Adapter.HelpListAdapter;
...
public class HelperFragment extends BaseFragment {
    ...
    private DelHelpFragment dialog = new DelHelpFragment();
    ...
    private void initView() {
        RetrofitManager.builder().getSendHelpList(App.getInstance().getPlayId(), "10", "1")
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Action1<HelpListBean>() {
                    @Override
                    public void call(HelpListBean helpListBean) {
                        if (helpListBean.getHelp().size() < 10){
    ...
    private void updateView() {
        RetrofitManager.builder().getSendHelpList(App.getInstance().getPlayId(), "10", page + "")
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Action1<HelpListBean>() {
                    @Override
    ...
    protected void initPageView(View rootView) {
        ...
        adapter = new HelpListAdapter(getActivity());
    ...
    private void initClick() {
        adapter.setOnLongItemClickListener(new HelpListAdapter.OnLongItemClickListener() {
            @Override
            public void onLongItemClick(View view, String helpId) {
                dialog.setHelpId(helpId);
                dialog.show(getFragmentManager(), "dialog");
            }
        });

        dialog.setOnConfirmClickListener(new DelHelpFragment.OnConfirmClickListener() {
            @Override
            public void onConfirmClick(String helpId) {
                RetrofitManager.builder().deleteHelp(helpId, App.getInstance().getPlayId())
                        .subscribeOn(Schedulers.io())
                        .observeOn(AndroidSchedulers.mainThread())
